...
public class Program
{
    private static async Task Main()
    {
        ...
        var connectionString = configuration["ShortUri:Database"];
        if (string.IsNullOrWhiteSpace(connectionString))
        {
            Console.ForegroundColor = ConsoleColor.Red;
            Console.WriteLine("错误: 未找到数据库连接字符串配置 (ShortUri:Database)");
            Console.ResetColor();
            return;
        }
...
#pragma warning disable EF1001 // Internal EF Core API usage.
            ...
            var codeOptions = new ModelCodeGenerationOptions
            {
                ...
                ContextDir = dataDir,
                ContextNamespace = "ShortUri.Database",
                ModelNamespace = "ShortUri.Database.Entities",
                RootNamespace = "ShortUri.Database",
    ...
    {
        var configurationFilePath = Path.Combine(Directory.GetParent(projectDirectory)!.Parent!.FullName, "src", "ShortUri", "appsettings.Development.json");
